$(function() {
	...
	var url = '/webshop/frontend/listmainpageinfo';
	// 访问后台,获取头条列表以及一级类别列表
	$
			.getJSON(
					url,
					function(data) {
						if (data.success) {
							// 获取后台传递过来的头条列表
							var headLineList = data.deadLineList;
							var swiperHtml = '';
							// 遍历头条列表,并拼接出轮播图组
							headLineList
									.map(function(item, index) {
										swiperHtml += ''
												+ '<div class="swiper-slide img-wrap"><a href="'
												+ item.lineLink
	...
	$('.row').on('click','.shop-classify',function(e){
		...
		var newUrl = '/webshop/frontend/shoplist?parentId=' + shopCategoryId;
