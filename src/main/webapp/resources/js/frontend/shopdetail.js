$(function(){
	...
	var pageSize = 3;
	//列出商品列表的URL
	毁掉listUrl = '/webshop/frontend/listproductsbyshop';
	//默认的页码
	var pageNum = 1;
	//从地址栏里获取ShopId
	...
	var searchDivUrl = '/webshop/frontend/listshopdetailpageinfo?shopId=' + shopId;
	...
	//$('#exchangelist').attr('href','/webshop/frontend/awardlist?shopId=' + shopId);
	//获取本店铺信息以及商品类别信息列表
		...
		$.getJSON(url,function(data){
			if(data.success){
				...
				productCategoryList.map(function(item,index){
					html += '<a href="#" class="button" data-product-id='
	...
	function addItems(pageSize,pageIndex){
		//拼接出查询的URL,赋空值默认就去掉这个条件限制,有值就代表按这个条件去查找
			...
			if(data.success){
				...
				data.productList.map(function(item,index){
					html += '' + '<div class="card" data-product-id=' + item.productId +'>'
						+ '<div class="card-header">' + item.productName +'</div><div class="card-content">'
						+ '<div class="list-block media-list"><ul><li class="item-content"><div class="item-media">'
						+ '<img src="' +item.imgAddr +'"width="44"></div><div class="item-inner">'
						+ '<div class="item-subtitle">'+ item.productDesc + '</div></div></li></ul></div></div>'
						+ '<div class="card-footer"><p class="color-gray">' + new Date(item.lastEditTime).Formate("yyyy-MM-dd")
...
//					$.detachInfiniteScroll($('.infinite-scroll'));
	...
	$('#shopdetail-button-div').on('click','.button',function(e){
		...
		if(productCategoryId){
			...
			if($(e.target).hasClass('button-fill')){
				$(e.targe).removeClass('button-fill');
				productCategoryId = '';
			}else{
	...
	$('.list-div').on('click','.card',function(e){
		...
		window.location.href = '/webshop/frontend/productdetail?productId=' + productId;
	});
